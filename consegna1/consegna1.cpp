...
int main(int argc, char **argv) {
    ...
    // creazione file input: NON USARE PIU' --> il file data.txt ufficiale � stato allegato, per permettere confronti equivalenti
    ...
    for (test = 0; test < ntests; test++) {
        ...
        if (read_max < 0 || read_max < ct_read)
            read_max = ct_read;
        printf("Test %d %d\n", test, ct_read);
        fprintf(myfile, "Test %d %d\n", test, ct_read);
    }

    printf("%d,%d,%.1f,%d\n", ntests, read_min, (0.0 + read_avg) / ntests, read_max);
    fprintf(myfile,"%d,%d,%.1f,%d\n", ntests, read_min, (0.0 + read_avg) / ntests, read_max);
    fclose(myfile);
    delete[] A;
